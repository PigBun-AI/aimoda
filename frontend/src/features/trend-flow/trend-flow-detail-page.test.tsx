...
vi.mock('@/features/trend-flow/use-trend-flow-detail', () => ({
  useTrendFlowDetail: (...args: Parameters<typeof useTrendFlowDetail>) => mockedUseTrendFlowDetail(...args),
}))
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter initialEntries={['/trend-flow/1']} future={ROUTER_FUTURE}>
          <Routes>
            <Route path="/trend-flow/:trendFlowId" element={<TrendFlowDetailPage />} />
          </Routes>
        </MemoryRouter>
      </QueryClientProvider>,
    )
