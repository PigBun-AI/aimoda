...
export function TrendFlowDetailPage() {
  ...
  const isLocked = trendFlowQuery.error instanceof ApiError && trendFlowQuery.error.status === 403
      ...
      return (
        <div className="flex h-full min-h-0 items-center justify-center bg-background px-6">
          <div className="w-full max-w-2xl border border-border bg-card px-8 py-10">
            ...
            <h1 className="type-page-title mt-3 text-foreground">{t('lockedTitle')}</h1>
  ...
  const formattedDate = new Date(trendFlowQuery.data.updatedAt).toLocaleDateString(
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4">
        <div className="shrink-0 border-b border-border/70 pb-4 sm:pb-5">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 pb-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/trend-flow')}>
              <ArrowLeft className="size-4" strokeWidth={1.75} />
              <span>{t('backToList')}</span>
            </Button>
            <div className="flex flex-wrap items-center gap-2 text-right">
              <span className="type-chat-kicker text-muted-foreground">{trendFlowQuery.data.brand}</span>
              <span className="type-chat-kicker border border-border px-2 py-1 text-muted-foreground">Trend Flow</span>
            </div>
          </div>
          ...
          <div className="pt-4 sm:pt-5">
            <PageIntro
              ...
              description={trendFlowQuery.data.leadExcerpt || t('detailDeck', { brand: trendFlowQuery.data.brand, window: trendFlowQuery.data.windowLabel })}
                ...
                <div className="flex h-full flex-col justify-between gap-4">
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-4 border-b border-border/60 pb-3">
                      ...
                      <span className="type-chat-meta text-right text-foreground">{trendFlowQuery.data.brand}</span>
                    </div>
                    <div className="flex items-start justify-between gap-4 border-b border-border/60 pb-3">
                      <span className="type-chat-kicker text-muted-foreground">Window</span>
                      <span className="type-chat-meta text-right text-foreground">{trendFlowQuery.data.windowLabel}</span>
                    </div>
                  ...
                  <div className="grid gap-2 border-t border-border/60 pt-3">
                    {trendFlowQuery.data.timeline.map((point) => (
                      <div key={`${point.year}-${point.quarter}`} className="flex items-center justify-between border border-border/60 bg-background px-3 py-2">
                        <span className="type-chat-kicker text-muted-foreground">{point.quarter}</span>
                        <span className="type-chat-meta tabular-nums text-foreground">{point.year}</span>
                      </div>
                    ))}
                    <Button asChild variant="outline" size="sm">
                      <Link to={trendFlowQuery.data.previewUrl} target="_blank" rel="noreferrer">
                        {t('openRawPreview')}
                      </Link>
                    </Button>
                  </div>
          ...
          <aside className="hidden border border-border bg-background px-4 py-4 xl:flex xl:flex-col xl:justify-between">
            <div className="space-y-4">
              <div className="border-b border-border pb-3">
                <p className="type-chat-kicker text-muted-foreground">Timeline</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {trendFlowQuery.data.timeline.map((point) => (
                    <span key={`${point.year}-${point.quarter}-chip`} className="type-chat-kicker border border-border/70 bg-card px-2.5 py-1 text-muted-foreground">
              ...
              <div className="space-y-2">
                ...
                <p className="type-chat-title text-foreground">{trendFlowQuery.data.brand}</p>
              </div>
              <div className="space-y-2 border-t border-border pt-3">
                <p className="type-chat-kicker text-muted-foreground">Window</p>
                <p className="type-chat-meta text-foreground">{trendFlowQuery.data.windowLabel}</p>
              </div>
            </div>
            <p className="type-chat-meta text-muted-foreground">Single-brand trend flow across four consecutive quarters.</p>
          </aside>

          <div className="min-h-[60vh] border border-border bg-background xl:min-h-0">
            {safeIframeUrl ? (
              <iframe className="h-full min-h-[60vh] w-full border-0 bg-white dark:bg-black xl:min-h-0" src={safeIframeUrl} title={trendFlowQuery.data.title} />
            ) : (
