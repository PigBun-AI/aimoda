...
const SEARCH_LINK_CLASS = 'type-chat-kicker mt-2 inline-flex items-center gap-1 border border-border px-2 py-1 text-muted-foreground transition-colors group-hover:border-foreground/30 group-hover:bg-accent/20 group-hover:text-foreground'
const COLOR_SWATCH_ROW_CLASS = 'flex w-full items-center justify-between border-b border-black/10 px-3 py-2 text-left transition-opacity last:border-b-0 hover:opacity-92'
        ...
        {(image.brand || locationInfo || descriptionText) && (
          <section className="space-y-4 border-b border-border pb-6">
            ...
            {image.brand && (
              <button
                type="button"
                className={cn('group inline-flex max-w-full items-start gap-3 text-left', isSearchingBrand && 'pointer-events-none opacity-60')}
              ...
              >
                <span className="min-w-0">
                  ...
                  <span className={SEARCH_LINK_CLASS}>
                    {t('searchAction')}
                    {isSearchingBrand ? <Loader2 className="size-3 animate-spin" /> : <ArrowUpRight className="size-3" />}
                  </span>
                </span>
              </button>
            )}
        ...
        {extractedColors.length > 0 && (
          <section className="space-y-5">
            ...
            <div className="relative">
              <div className={cn('flex flex-col border border-border', isSearchingColor && 'pointer-events-none opacity-50')}>
                {extractedColors.slice(0, 8).map((color, index) => {
                  ...
                  return (
                    <button
                      key={index}
                      type="button"
                      className={COLOR_SWATCH_ROW_CLASS}
