# AiModa Fashion Report Zip Spec v2
...
- `coverImage`: 必填，封面图相对路径；推荐使用 `assets/cover.jpg`
...
- `coverImage` 是必填的
- 封面图请随报告包一起上传，推荐使用 `assets/cover.jpg`
- `coverImage` 必须是报告第一页/首屏的截图，不是任意独立海报图
...
- [ ] `manifest.json` 中的 `coverImage` 指向真实文件
- [ ] `coverImage` 内容与报告第一页/首屏一致
- [ ] 所有 HTML/CSS/JS/图片引用均为 zip 内相对路径
- [ ] 正文图片未以内联 base64 大图方式嵌入
- [ ] `slug`、`title`、`brand`、`season`、`year` 已填写
- [ ] `coverImage`、`featuresFile`、`pages` 中声明的路径都真实存在
...
- 平台会直接使用 `manifest.json.coverImage` 作为报告封面
- `overviewHtml` 缺失不会阻止上传
- 没有 `manifest.json` 时，平台仅兼容旧格式 `index.html + cover.jpg`
