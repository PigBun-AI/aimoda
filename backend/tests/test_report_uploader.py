...
class FakeOSS:
    ...
    def upload_file(self, oss_path, file_content, content_type=None, metadata=None):
        if isinstance(file_content, bytes):
            content = file_content
        else:
            content = file_content.read()
        self.uploads.append((oss_path, content_type, content))
        return f"https://oss.example.com/{oss_path}"
...
def test_upload_report_preserves_relative_paths_and_uses_manifest_entry(tmp_path, monkeypatch):
    ...
    uploaded_paths = {path for path, _content_type, _content in fake_oss.uploads}
