...
async def test_search_session_endpoint_uses_total_count_and_paged_results(monkeypatch):
    ...
    monkeypatch.setattr(
        chat_router,
        "get_session_page",
        lambda client, session, offset=0, limit=20: [
            type("Point", (), {"payload": {"image_id": f"img-{offset + 1}"}, "score": 0.91})(),
            type("Point", (), {"payload": {"image_id": f"img-{offset + 2}"}, "score": 0.89})(),
        ],
    )
    monkeypatch.setattr(
        chat_router,
    ...
    assert [item["image_id"] for item in response["images"]] == ["img-201", "img-202"]
